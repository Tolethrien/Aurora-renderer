...
export default class ColorCorrection {
  ...
  public static controller(
    ...
    slider.addEventListener("input", (event) => {
      ...
      this.options[DataAcces[opt]] = Number(target.value);
  ...
  public static tinter() {
    const indexStart = DataAcces["tint"];
    Array(4)
      .fill(0)
      .forEach((_, index) => {
        const color = ["TingRed", "TingGreen", "TingBlue", "TingStr"];
        const window = document.getElementById("sdrgr")!;
        ...
        slider.step = String(0.01);
        slider.addEventListener("input", (event) => {
          const target = event.target as HTMLInputElement; // Cast to HTMLInputElement to access .value
  ...
  public static usePipeline() {
    ...
    const { meta } = Renderer.getTexture("lightMap");
    const size = {
    ...
    AuroraDebugInfo.accumulate("pipelineInUse", ["colorCorrection"]);
  }
