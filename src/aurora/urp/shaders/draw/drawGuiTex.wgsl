...
@group(1) @binding(0) var<storage> text: FormattedText;

//textElement = {x,y,charIndex}
@vertex
fn vertexMain(input : VertexInput) -> VertexOutput {
  let textElement = text.chars[input.instance];
  let char = chars[u32(textElement.z)];
  // let charPos = (pos[input.vertex]); 
  let charPos = (pos[input.vertex] * char.size + textElement.xy + 0) * text.transform.z; // Użyj .x, bo skala jest w pierwszym komponencie
  var output : VertexOutput;
  
  
  output.position = vec4f(text.transform.xy + charPos, 0, 1); 
